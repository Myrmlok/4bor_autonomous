...
export default function Liquidation() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-serif font-semibold mb-2 text-destructive">Ликвидация</h1>
        <p className="text-muted-foreground">Распродажа лотов по сниженным ценам</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredLots.map(lot => (
            <Card key={lot.id} className="overflow-hidden flex flex-col hover-elevate border-destructive/20">
              <Link href={`/lots/${lot.id}`} className="relative h-48 overflow-hidden block group">
                ...
                {lot.format === 'auction' && (
                   <Badge className="absolute top-3 right-3 bg-black/60 text-white hover:bg-black/60 border-none">
                     Аукцион
                   </Badge>
                )}
              </Link>
                ...
                <div className="flex items-center justify-between mt-auto pt-2 border-t">
                  <div>
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">
                      {lot.format === 'auction' ? 'Ставка' : 'Цена'}
                    </div>
                    <div className="font-semibold text-destructive">{formatPrice(lot.price || lot.bidMax || lot.bidMin)}</div>
                  </div>
                  {lot.format === 'auction' && (
