    ...
    return (
      <div className="p-8 max-w-4xl mx-auto">
        <div className="py-20 text-center border border-border/50 bg-card">
          ...
          <h2 className="text-2xl font-serif font-semibold mb-3">Заявка оформлена</h2>
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-serif font-semibold">Корзина</h1>
        {items.length > 0 && (
          <span className="text-sm text-muted-foreground">{items.length} {items.length === 1 ? 'лот' : 'лота'}</span>
        )}
      </div>

      {/* [STUB] Уведомление о заглушке оплаты */}
      {items.length > 0 && (
        <div className="flex items-start gap-3 border border-amber-300/40 bg-amber-50/50 px-4 py-3 mb-6 text-sm text-amber-800">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-4">
            {items.map(({ lot }) => (
              <Card key={lot.id} className="overflow-hidden flex border-border/50">
                <Link href={`/lots/${lot.id}`} className="w-32 h-28 flex-shrink-0 bg-muted block">
                ...
                <CardContent className="p-4 flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      <Link href={`/lots/${lot.id}`} className="hover:text-primary transition-colors">
                        <h3 className="font-serif font-medium leading-tight mb-1">{lot.title}</h3>
                      </Link>
                      <div className="text-xs text-muted-foreground">
                        Лот #{lot.id.toUpperCase()} · {lot.format === 'fixed' ? 'Фиксированная цена' : 'Аукцион'}
                      </div>
                    </div>
                    <button
                  ...
                  <div className="font-semibold text-lg mt-2">{formatPrice(lot.price || lot.bidMax)}</div>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <div className="w-full lg:w-80">
            <Card className="p-6 sticky top-24 border-border/50">
              <h2 className="font-serif text-xl font-medium mb-4 border-b border-border/50 pb-4">Ваш заказ</h2>
              ...
              <div className="flex justify-between items-center mb-6 pt-4 border-t border-border/50">
