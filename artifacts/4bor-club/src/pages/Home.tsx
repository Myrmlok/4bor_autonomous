    ...
    <div className="flex flex-col pb-20">
      ...
      <section className="relative h-[180px] bg-secondary flex flex-col justify-center px-8 overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/70 font-light tracking-wide">Закрытое пространство для дилеров и коллекционеров</p>
        </div>
      </section>

      {/* Themes */}
      <section className="px-8 py-10">
        <h2 className="text-xs font-bold tracking-widest text-muted-foreground uppercase mb-6">Тематики</h2>
          ...
          {themes.map(theme => {
            ...
            return (
              <button
                ...
                className={`group relative flex-shrink-0 w-48 h-64 overflow-hidden snap-start transition-all duration-300 ${
                  isActive ? 'ring-2 ring-primary ring-offset-4 ring-offset-background' : 'opacity-75 hover:opacity-100 hover:-translate-y-1'
                }`}
              >
      ...
      {/* Active Theme Groups table */}
      <section className="px-8 pb-12">
        <div className="bg-card border border-border/50 overflow-hidden">
          <div className="px-6 py-4 border-b border-border/50 bg-muted/30">
            <h2 className="font-serif text-2xl font-semibold">{activeTheme.name}</h2>
          </div>
          <div className="divide-y divide-border/40">
            {themeGroups.map(group => (
              <div key={group.id} className="flex flex-col md:flex-row items-stretch hover:bg-muted/10 transition-colors">
                <div className="w-full md:w-1/4 px-6 py-4 border-b md:border-b-0 md:border-r border-border/30 font-medium text-sm flex items-center">
                  {group.name}
                </div>
                <div className="w-full md:w-3/4 grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border/30">
                  {/* Аукционы — доступно всем */}
                  <Link
                  ...
                  {/* Эксклюзивы — только дилерам/админу */}
                  {isCollector ? (
                    <div className="flex items-center justify-between px-6 py-4 text-muted-foreground/40 cursor-not-allowed select-none">
                  ...
                  {/* Ликвидация — доступно всем */}
                  <Link
                    href={`/catalog/${activeTheme.id}/groups/${group.id}?section=liquidation`}
                    className="flex items-center justify-between px-6 py-4 group/link hover:text-destructive transition-colors"
                  >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          <Link href="/auctions" className="p-4 border border-border/50 bg-card hover:border-primary/50 transition-colors group">
          ...
          {isCollector ? (
            <div className="p-4 border border-border/30 bg-muted/20 text-muted-foreground/40 cursor-not-allowed select-none">
            ...
            <Link href="/exclusives" className="p-4 border border-border/50 bg-card hover:border-primary/50 transition-colors group">
              <div className="text-primary font-mono text-xs mb-2">02</div>
              <div className="font-medium group-hover:text-primary transition-colors text-sm">Эксклюзивы</div>
            </Link>
          )}

          <Link href="/liquidation" className="p-4 border border-border/50 bg-card hover:border-destructive/40 transition-colors group">
            <div className="text-destructive font-mono text-xs mb-2">03</div>
            <div className="font-medium group-hover:text-destructive transition-colors text-sm">Ликвидация</div>
          </Link>

          {/* [STUB] Архив — раздел в разработке */}
          <div className="p-4 border border-border/30 bg-muted/20 text-muted-foreground/40 cursor-not-allowed select-none" title="В разработке">
            <div className="flex justify-between items-start mb-2">
            ...
            {newsList.map(news => (
              <Link key={news.id} href={`/news/${news.id}`} className="group flex flex-col gap-3">
                ...
                <div>
                  ...
                  <h3 className="font-serif font-medium leading-tight group-hover:text-primary transition-colors line-clamp-2 text-sm">
                    {news.title}
                  </h3>
                </div>
              </Link>
