// Static catalog data (mirrors 4bor-club/src/data/mock.ts).
// Replace with DB queries when admin lot management is implemented.
...
export interface Lot {
  ...
  format: LotFormat; status: LotStatus; imageUrl: string;
  themeId: string; groupId: string;
  sectionType: 'auction' | 'exclusive' | 'liquidation';
  createdAt: string;
}
export interface News {
  id: string; date: string; title: string; imageUrl: string;
}
export interface Activity {
  id: string; text: string; timeAgo: string;
}
export interface Sticker {
  id: string; userId: number; text: string; budget: number; imageUrl: string; createdAt: string;
}
